...
if [ $RES = 1 ];then
    ...
    echo -e ""
    echo -e "${bldblu}External out directory is set to: ${bldgrn}($OUTDIR)${txtrst}"
    echo -e ""
elif [ $RES = 0 ];then
    export OUTDIR=$DIR/out
    echo -e ""
    echo -e "${bldblu}No external out, using default: ${bldgrn}($OUTDIR)${txtrst}"
    echo -e ""
else
    echo -e ""
    echo -e "${bldred}NULL${txtrst}"
    echo -e "${bldred}Error, wrong results! Blame the split screen!${txtrst}"
    echo -e ""
fi
...
echo -e "${bldblu}Building ${bldgrn}P ${bldred}A ${bldblu}C ${bldylw}$PAC_VERSION${txtrst}"
