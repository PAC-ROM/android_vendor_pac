...
# and install ncftpput: sudo apt-get install ncftpput
if [ "$opt_upload" -ne 0 ]; then
    uploadmd5="$OUTDIR"/target/product/"$device"/pac*.md5sum
    uploadzip="$OUTDIR"/target/product/"$device"/pac*.zip
    echo ""
    if [ -e "uploadmd5" ] && [ -e "uploadzip" ]; then
        server=$(cat ~/Server)
        suser=$(awk -F'::' '{ print $1 }' "$server")
        spass=$(awk -F'::' '{ print $2 }' "$server")
        shost=$(awk -F'::' '{ print $3 }' "$server")
        spath=$(awk -F'::' '{ print $4 }' "$server")
        echo -e "${bldgrn}Uploading ROM to ${shost} as ${suser} at ${spath}"
        ncftpput -v -u "${suser}" -p "${spass}" ${shost} ${spath} "$uploadmd5"
        ncftpput -v -u "${suser}" -p "${spass}" ${shost} ${spath} "$uploadzip"
        echo -e "Upload Complet${rst}"
    else
        echo -e "The ROM it does not exist${rst}"
    fi
    echo ""
fi
