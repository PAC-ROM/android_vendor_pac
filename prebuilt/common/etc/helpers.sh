...
is_busybox_applet_available() {
    ...
    if busybox [ $exit_code -eq 0 ]; then
        for i in $applets; do
            if busybox [ $applet == $i ]; then
                return 0;
            fi;
        done;
    fi;
...
zipalign_apk() {
    ...
    if busybox [ -e $apk ]; then
        ...
        case $exit_status in
            ...
            ;;
            *)
                if zipalign -f 4 $apk /data/local/pkg.apk; then
...
============================================================= '
zipalign_apks() {
    if ! is_binary_installed zipalign; then
